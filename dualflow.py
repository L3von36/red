...
# ─────────────────────────────────────────────────────────────────────────────
# Phase 1 has better overall speed accuracy (2.694 km/h).
# Phase 2 has better jam detection (15.329 km/h jam MAE, higher R²).
# Blend: during free-flow → trust Phase 1; during congestion → trust Phase 2.
# Gating signal: Phase 2's jam logit (it specialised in jam classification).
# ═══════════════════════════════════════════════════════════════════════════════

class JamAwareEnsemble(nn.Module):
    """Blends Phase 1 (speed expert) and Phase 2 (jam expert) via learned gating."""
    def __init__(self, phase1_net, phase2_net, hidden=64):
        super().__init__()
        self.phase1 = phase1_net
        self.phase2 = phase2_net
        # Learns a per-node temperature to sharpen/soften the jam-prob gate
        self.gate_temp = nn.Parameter(torch.ones(1))
        # Optional: learnable global blend bias (initialised neutral)
        self.gate_bias = nn.Parameter(torch.zeros(1))

    def _jam_gate(self, jam_logit):
        """Convert Phase 2 jam logit to a smooth [0,1] gate (0=free, 1=jam)."""
        return torch.sigmoid(self.gate_temp * jam_logit + self.gate_bias)

    @torch.no_grad()
    def impute(self, x, m, tod_free=None, tod_jam=None):
        """Jam-aware imputation: Phase 1 for free-flow, Phase 2 for congestion."""
        # Phase 1: accurate speed imputation
        p1 = self.phase1._run(x, m, tod_free, tod_jam)
        p1 = torch.clamp(p1, -5.0, 5.0)

        # Phase 2: jam-specialised speed + jam logit
        p2_speed, p2_jam_logit = self.phase2._run_full(x, m, tod_free, tod_jam)
        p2_speed = torch.clamp(p2_speed, -5.0, 5.0)

        # Gate: high jam probability → lean on Phase 2 predictions
        gate = self._jam_gate(p2_jam_logit)          # [N, T] in [0, 1]
        blended = (1.0 - gate) * p1 + gate * p2_speed  # [N, T]

        # Anchor-diffusion on blended prediction (use Phase 1 setting)
        p = m * x + (1.0 - m) * blended
        for _ in range(3):
            p_smooth = A_t @ p
            p_new    = 0.7 * p + 0.3 * p_smooth
            p = m * x + (1.0 - m) * p_new
        return p

    def forward(self, x, m, tod_free=None, tod_jam=None):
        return self.impute(x, m, tod_free, tod_jam)


def build_jam_aware_ensemble(phase1_net, phase2_net):
    """Construct the hybrid model; both sub-nets stay frozen."""
    for p in phase1_net.parameters():
        p.requires_grad_(False)
    for p in phase2_net.parameters():
        p.requires_grad_(False)
    phase1_net.eval()
    phase2_net.eval()
    ensemble = JamAwareEnsemble(phase1_net, phase2_net).to(device)
    return ensemble


print("✅ JamAwareEnsemble (Phase 1 speed + Phase 2 jam) defined")
